...
public class WordCount3 extends Configuration implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        for (URI cacheFile : cacheFiles) {
            ...
            //캐시 메모리에 업로드된 파일명 확인
            log.info("Upload CacheFile : " + uploadFile);

        }

        // 분석할 폴더 메모리에 올라간 /comedies 파일명 작성함
        // 메모리에 존재하면, 그 파일을 먼저 ㅂ다음
        ...
        //미니 리듀서라 부르며, Conbiner 수행 설정
        //보통 Reducer 역할을 수행하는 객체를 바인딩 함
        job.setCombinerClass(WordCount3Reducer.class);

        //분석 결과가 저장될 때 사용될 값의 데이터 타입
        job.setOutputKeyClass(Text.class);

        //분석 결과가 저장될 대 사용될 값의 데이터 타입
