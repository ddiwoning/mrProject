...
/**
 * 맵 역할을 수행하기 위해서는 Mapper자바 파일을 상속받아야 함
...
public class MonthLog2Mapper extends Mapper<LongWritable, Text, Text, Text> {
    ...
    //access_log파일로부터 추출될 정보가 제대로 수집되었는지 확인하기 위해 만듬
    ...
    public void map(LongWritable key, Text value, Context context)
        ...
        String ip = fields[0]; // IP호출
        String reqTime = ""; // 요청일시

        if (fields[3].length() > 2) { //일부 데이터가 요청일시 값이 누락된 경우가 있어 요청일시 값이 존재해ㅏ는지 체크
        ...
        String reqURI = fields[6]; // 요청일시에서 월 정보 추출하기
        ...
        if (months.contains(reqMonth)) {
            ...
            //MonthLog2Partitioner 로 보내서, 웗려 리듀스 분할하기
